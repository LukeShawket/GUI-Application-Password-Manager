...
with open("directory_data.txt", "a+") as direct_data:
    direct_data.seek(0)
...
def choose_directory():
    ...
    if directory:
        final_directory = f"{directory}/my_passwords.txt"
        directory_entry.delete(0, END)
        directory_entry.insert(0, final_directory)
        with open("directory_data.txt", "w+") as direct_data:
...
def save_password():
    ...
    formated = (f"Website name or App name: {web_name}\n"
...
def generate_password_pressed():
    ...
    number_label = Label(password_window, text="How many numbers letters?")
    ...
    symbol_label = Label(password_window, text="How many symbols letters?")
...
password_button.grid(row=5, column=1, pady=5)

add_button = Button(text="Add", width=9, pady=0, bg="white", command=save_password)
add_button.grid(row=5, column=2, columnspan=2, pady=5)
